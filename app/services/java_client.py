        ...
        try:
            async with httpx.AsyncClient(timeout=JavaServiceClient.TIMEOUT) as client:
                ...
                if response.status_code == 200:
                    logger.info(f"✅ Sala {sala_id} validada exitosamente desde Java Service")
                    return True
                elif response.status_code == 404:
                    logger.warning(f"⚠️ Sala {sala_id} no encontrada en Java Service")
                    return False
                else:
                    logger.error(f"❌ Error al validar sala {sala_id}: Status {response.status_code}")
                    return False
        ...
        try:
            async with httpx.AsyncClient(timeout=JavaServiceClient.TIMEOUT) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"✅ Detalles de sala {sala_id} obtenidos desde Java Service")
                    return sala_data
                else:
                    logger.warning(f"⚠️ No se pudieron obtener detalles de sala {sala_id}")
                    return None
    ...
    async def get_salas_disponibles() -> list:
        ...
        try:
            async with httpx.AsyncClient(timeout=JavaServiceClient.TIMEOUT) as client:
                ...
                if response.status_code == 200:
                    salas = response.json()
                    logger.info(f"✅ {len(salas)} salas disponibles obtenidas desde Java Service")
                    return salas
                else:
                    logger.warning(f"⚠️ Error al obtener salas disponibles: {response.status_code}")
                    return []
    ...
    async def validate_articulo_exists(articulo_id: int) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=JavaServiceClient.TIMEOUT) as client:
                ...
                if response.status_code == 200:
                    logger.info(f"✅ Artículo {articulo_id} validado exitosamente desde Java Service")
                    return True
                elif response.status_code == 404:
                    logger.warning(f"⚠️ Artículo {articulo_id} no encontrado en Java Service")
                    return False
                else:
                    logger.error(f"❌ Error al validar artículo {articulo_id}: Status {response.status_code}")
                    return False
        ...
        try:
            async with httpx.AsyncClient(timeout=JavaServiceClient.TIMEOUT) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"✅ Detalles de artículo {articulo_id} obtenidos desde Java Service")
                    return articulo_data
                else:
                    logger.warning(f"⚠️ No se pudieron obtener detalles de artículo {articulo_id}")
                    return None
                    
        except Exception as e:
            logger.error(f"❌ Error al obtener detalles de artículo {articulo_id}: {str(e)}")
            return None
        ...
        try:
            async with httpx.AsyncClient(timeout=2.0) as client:
                ...
                if is_healthy:
                    ...
                    logger.warning(f"⚠️ Java Service respondió con status {response.status_code}")
